...
func (s *testTimeFallBackSuite) SetUpSuite(c *C) {
	s.ctx, s.cancel = context.WithCancel(context.Background())
	c.Assert(failpoint.Enable("github.com/pingcap/pd/v3/server/tso/fallBackSync", `return(true)`), IsNil)
	c.Assert(failpoint.Enable("github.com/pingcap/pd/v3/server/tso/fallBackUpdate", `return(true)`), IsNil)
	var err error
	...
	svr.Close()
	failpoint.Disable("github.com/pingcap/pd/v3/server/tso/fallBackSync")
	failpoint.Disable("github.com/pingcap/pd/v3/server/tso/fallBackUpdate")
	err = svr.Run()
	c.Assert(err, IsNil)
	s.cluster.WaitLeader()
}
...
func (s *testFollowerTsoSuite) TestRequest(c *C) {
	c.Assert(failpoint.Enable("github.com/pingcap/pd/v3/server/tso/skipRetryGetTS", `return(true)`), IsNil)
	var err error
	...
	failpoint.Disable("github.com/pingcap/pd/v3/server/tso/skipRetryGetTS")
}
