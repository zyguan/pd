...
func (s *joinTestSuite) TestFailedPDJoinInStep1(c *C) {
	...
	// Join the second PD.
	c.Assert(failpoint.Enable("github.com/pingcap/pd/v3/server/join/add-member-failed", `return`), IsNil)
	_, err = cluster.Join(ctx)
	c.Assert(err, NotNil)
	c.Assert(strings.Contains(err.Error(), "join failed"), IsTrue)
	c.Assert(failpoint.Disable("github.com/pingcap/pd/v3/server/join/add-member-failed"), IsNil)
}
